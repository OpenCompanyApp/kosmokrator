...
final class PhaseStateMachine
{
    ...
    {
        // Signal<T> is invariant in T (has both get and set), so PHPStan
        // widens `new Signal(Phase::Idle)` to `Signal<Phase::Idle>`.
        // The machine only writes Phase values, so this is safe.
        $this->signal = $signal ?? new Signal(Phase::Idle); // @phpstan-ignore assign.propertyType
    ...
    {
        return $this->signal; // @phpstan-ignore return.type
    }
