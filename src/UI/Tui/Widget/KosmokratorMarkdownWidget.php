 ...
 */
class KosmokratorMarkdownWidget extends AbstractWidget
{
    private static ?MarkdownParser $sharedParser = null;

    private static ?Highlighter $sharedHighlighter = null;
    ...
    private ?string $materializedText = null;

    private ?string $lastRenderHash = null;

    private ?int $lastRenderColumns = null;

    /** @var string[]|null */
    private ?array $lastRenderLines = null;
    ...
    public function __construct(
        ...
        $this->chunks = [StringUtils::sanitizeUtf8($text)];
        if ($parser === null) {
            $parser = self::$sharedParser ??= $this->createParser();
        }
        $this->parser = $parser;

        if ($highlighter === null && ! class_exists(Highlighter::class)) {
        ...
        $this->highlighter = $highlighter ?? (self::$sharedHighlighter ??= new Highlighter(new DarkTerminalTheme));
    }
    ...
    {
        $this->chunks = [StringUtils::sanitizeUtf8($text)];
        $this->materializedText = null;
        $this->lastRenderHash = null;
        $this->lastRenderColumns = null;
        $this->lastRenderLines = null;
        $this->invalidate();
    ...
    {
        return $this->materializedText ??= implode('', $this->chunks);
    }
    ...
    private function renderMarkdown(RenderContext $context): array
    {
        ...
        if ($this->lastRenderHash === $hash
            && $this->lastRenderColumns === $contentColumns
            && $this->lastRenderLines !== null) {
            return $this->lastRenderLines;
        }
        ...
        $this->lastRenderColumns = $contentColumns;
        $this->lastRenderLines = $wrappedLines;
    ...
    private function renderCodeBlock(string $code, ?string $language, int $columns): array
    {
        ...
        $source = $highlighted ?? $code;
        foreach (explode("\n", $source) as $line) {
            if (AnsiUtils::visibleWidth($line) > $availableColumns) {
                foreach (TextWrapper::wrapTextWithAnsi($line, $availableColumns) as $wrapped) {
