<?php
/**
 * Shared layout for all pages.
 *
 * Expects: $pageTitle, $pageBody, $pageClass (optional), $extraHead (optional), $extraScript (optional)
 */
$pageTitle ??= 'KosmoKrator — AI Coding Agent for the Terminal';
$pageClass ??= '';
$extraHead ??= '';
$extraScript ??= '';
$isDocsPage = str_contains($pageClass, 'docs-page');
?>
<!DOCTYPE html>
<html lang="en" style="background:#07070d;">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= htmlspecialchars($pageTitle) ?></title>

    <!-- SEO -->
    <meta name="description" content="Lightweight, mythology-themed AI coding agent written in PHP. ~50MB RAM, 40+ LLM providers, parallel subagent swarms, and a permission system that keeps you in control.">
    <meta name="keywords" content="AI coding agent, terminal, CLI, PHP, LLM, code generation, subagents, swarm, OpenAI, Anthropic, DeepSeek, open source">
    <meta name="author" content="OpenCompany">
    <meta name="robots" content="index, follow">

    <!-- Open Graph -->
    <meta property="og:type" content="website">
    <meta property="og:title" content="<?= htmlspecialchars($pageTitle) ?>">
    <meta property="og:description" content="Lightweight PHP coding agent with parallel subagent swarms, 40+ LLM providers, and ~50MB RAM footprint. Open source, MIT licensed.">
    <meta property="og:url" content="https://kosmokrator.dev/">
    <meta property="og:image" content="/og-image">
    <meta property="og:image:type" content="image/svg+xml">
    <meta property="og:image:width" content="1200">
    <meta property="og:image:height" content="630">
    <meta property="og:site_name" content="KosmoKrator">

    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:title" content="<?= htmlspecialchars($pageTitle) ?>">
    <meta name="twitter:description" content="Lightweight PHP coding agent with parallel subagent swarms, 40+ LLM providers, and ~50MB RAM footprint.">
    <meta name="twitter:image" content="/og-image">

    <!-- Favicon -->
    <link rel="icon" type="image/svg+xml" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'><rect width='64' height='64' rx='14' fill='%2307070d'/><text x='32' y='48' text-anchor='middle' font-size='44' font-weight='800' fill='%23dc143c' font-family='Inter,sans-serif'>K</text></svg>">
    <link rel="apple-touch-icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 180 180'><rect width='180' height='180' rx='40' fill='%2307070d'/><text x='90' y='130' text-anchor='middle' font-size='120' font-weight='800' fill='%23dc143c' font-family='Inter,sans-serif'>K</text></svg>">
    <meta name="theme-color" content="#dc143c">

    <?= $extraHead ?>

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;600;700&family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">

    <!-- Bootstrap 5 CSS -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">

    <style>
        /* ── Custom properties ── */
        :root {
            --space-black: #07070d;
            --space-dark: #0e0e18;
            --space-mid: #161625;
            --crimson: #dc143c;
            --crimson-light: #ff2d55;
            --crimson-glow: rgba(220, 20, 60, 0.4);
            --crimson-dim: rgba(220, 20, 60, 0.12);
            --crimson-subtle: rgba(220, 20, 60, 0.06);
            --star-white: #f0f0f5;
            --text-muted: rgba(240, 240, 245, 0.55);
            --text-dim: rgba(240, 240, 245, 0.35);
            --terminal-green: #00ff88;
            --surface: rgba(22, 22, 37, 0.6);
            --surface-hover: rgba(30, 30, 50, 0.8);
            --border: rgba(220, 20, 60, 0.1);
            --border-hover: rgba(220, 20, 60, 0.35);
        }

        /* ── Base resets over Bootstrap ── */
        html {
            scroll-behavior: smooth;
            scroll-padding-top: 5rem;
        }

        body {
            font-family: 'Inter', -apple-system, BlinkMacSystemFont, sans-serif;
            background: var(--space-black);
            color: var(--star-white);
            line-height: 1.7;
            overflow-x: hidden;
        }

        #starfield {
            position: fixed;
            inset: 0;
            z-index: 0;
            pointer-events: none;
        }

        /* ── Navigation (Bootstrap navbar + custom glassmorphism) ── */
        .navbar-kosmo {
            padding: 1rem 2rem;
            background: rgba(7, 7, 13, 0.7);
            backdrop-filter: blur(24px);
            -webkit-backdrop-filter: blur(24px);
            border-bottom: 1px solid transparent;
            transition: all 0.4s cubic-bezier(0.4, 0, 0.2, 1);
            z-index: 100;
        }

        .navbar-kosmo.scrolled {
            padding-top: 0.6rem;
            padding-bottom: 0.6rem;
            background: rgba(7, 7, 13, 0.92);
            border-bottom-color: var(--border);
        }

        .navbar-kosmo > .container-xl {
            max-width: 1200px;
        }

        .nav-brand {
            display: flex;
            align-items: center;
            gap: 0.6rem;
            text-decoration: none;
            color: var(--star-white);
            font-family: 'JetBrains Mono', monospace;
            font-weight: 700;
            font-size: 1.1rem;
            transition: opacity 0.2s;
        }

        .nav-brand:hover { opacity: 0.85; color: var(--star-white); }

        /* Custom hamburger bars inside Bootstrap toggler */
        .navbar-kosmo .navbar-toggler {
            border: none;
            padding: 4px;
            box-shadow: none;
        }

        .navbar-kosmo .navbar-toggler:focus { box-shadow: none; }

        .navbar-kosmo .navbar-toggler-icon {
            background-image: none;
            display: flex;
            flex-direction: column;
            gap: 5px;
            width: 22px;
            height: auto;
        }

        .navbar-kosmo .toggler-bar {
            display: block;
            width: 22px;
            height: 2px;
            background: var(--star-white);
            border-radius: 2px;
            transition: all 0.3s;
        }

        .navbar-kosmo .navbar-toggler[aria-expanded="true"] .toggler-bar:nth-child(1) {
            transform: rotate(45deg) translate(5px, 5px);
        }
        .navbar-kosmo .navbar-toggler[aria-expanded="true"] .toggler-bar:nth-child(2) {
            opacity: 0;
        }
        .navbar-kosmo .navbar-toggler[aria-expanded="true"] .toggler-bar:nth-child(3) {
            transform: rotate(-45deg) translate(5px, -5px);
        }

        .navbar-kosmo .navbar-collapse { flex-grow: 0; }

        .navbar-kosmo .navbar-nav { gap: 2.5rem; }

        .navbar-kosmo .nav-link {
            color: var(--text-muted) !important;
            font-size: 0.9rem;
            font-weight: 500;
            transition: color 0.2s;
            position: relative;
            padding: 0;
        }

        .navbar-kosmo .nav-link::after {
            content: '';
            position: absolute;
            bottom: -4px;
            left: 0;
            width: 0;
            height: 2px;
            background: var(--crimson);
            transition: width 0.3s ease;
        }

        .navbar-kosmo .nav-link:hover { color: var(--crimson-light) !important; }
        .navbar-kosmo .nav-link:hover::after { width: 100%; }
        .navbar-kosmo .nav-link.active { color: var(--crimson-light) !important; }
        .navbar-kosmo .nav-link.active::after { width: 100%; }

        /* Mobile collapse panel */
        @media (max-width: 991.98px) {
            .navbar-kosmo .navbar-collapse {
                background: rgba(7, 7, 13, 0.97);
                backdrop-filter: blur(20px);
                padding: 1.5rem 2rem;
                margin: 0.5rem -2rem 0;
                border-bottom: 1px solid var(--border);
            }
            .navbar-kosmo .navbar-nav { gap: 1rem; }
            .navbar-kosmo .nav-link { padding: 0.25rem 0; }
        }

        /* ── Container (override Bootstrap responsive widths) ── */
        .container {
            max-width: 1200px;
            margin: 0 auto;
            padding: 0 2rem;
            position: relative;
            z-index: 1;
            width: 100%;
        }

        /* ── Hero ── */
        .hero {
            min-height: 100vh;
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
            text-align: center;
            padding: 8rem 2rem 4rem;
            position: relative;
        }

        .hero::after {
            content: '';
            position: absolute;
            bottom: 0;
            left: 0;
            right: 0;
            height: 200px;
            background: linear-gradient(to bottom, transparent, var(--space-black));
            pointer-events: none;
            z-index: 1;
        }

        .nebula-glow {
            position: absolute;
            width: 600px;
            height: 600px;
            top: 50%;
            left: 50%;
            transform: translate(-50%, -55%);
            background: radial-gradient(ellipse at center, rgba(220, 20, 60, 0.08) 0%, rgba(220, 20, 60, 0.03) 30%, transparent 70%);
            pointer-events: none;
            z-index: 0;
            animation: nebula-pulse 8s ease-in-out infinite;
        }

        @keyframes nebula-pulse {
            0%, 100% { opacity: 0.7; transform: translate(-50%, -55%) scale(1); }
            50% { opacity: 1; transform: translate(-50%, -55%) scale(1.08); }
        }

        .logo-symbol {
            font-size: 5rem;
            color: var(--crimson);
            margin-bottom: 1.5rem;
            text-shadow: 0 0 60px var(--crimson-glow), 0 0 120px rgba(220, 20, 60, 0.2);
            animation: pulse-glow 4s ease-in-out infinite;
            position: relative;
            z-index: 2;
        }

        @keyframes pulse-glow {
            0%, 100% { text-shadow: 0 0 60px var(--crimson-glow), 0 0 120px rgba(220, 20, 60, 0.15); filter: brightness(1); }
            50% { text-shadow: 0 0 80px var(--crimson-glow), 0 0 160px rgba(220, 20, 60, 0.3); filter: brightness(1.1); }
        }

        .ascii-logo {
            font-family: 'JetBrains Mono', monospace;
            font-size: clamp(0.4rem, 1.1vw, 0.9rem);
            line-height: 1.25;
            color: var(--crimson);
            white-space: pre;
            overflow-x: auto;
            margin-bottom: 1.5rem;
            text-shadow: 0 0 40px var(--crimson-glow);
            opacity: 0;
            animation: logo-fade-in 1.2s ease 0.3s forwards;
            position: relative;
            z-index: 2;
        }

        @keyframes logo-fade-in {
            from { opacity: 0; transform: scale(0.95); }
            to { opacity: 0.9; transform: scale(1); }
        }

        h1 {
            font-family: 'Inter', sans-serif;
            font-size: clamp(3rem, 7vw, 5.5rem);
            font-weight: 800;
            background: linear-gradient(135deg, #ff6b81 0%, var(--crimson-light) 30%, var(--crimson) 60%, #a30025 100%);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-clip: text;
            margin-bottom: 1.5rem;
            letter-spacing: -0.04em;
            position: relative;
            z-index: 2;
            opacity: 0;
            animation: hero-fade-in 0.8s ease 0.6s forwards;
        }

        @keyframes hero-fade-in {
            from { opacity: 0; transform: translateY(20px); }
            to { opacity: 1; transform: translateY(0); }
        }

        .tagline {
            font-size: clamp(1.05rem, 2vw, 1.4rem);
            color: var(--text-muted);
            max-width: 640px;
            margin: 0 auto 3rem;
            line-height: 1.8;
            position: relative;
            z-index: 2;
            opacity: 0;
            animation: hero-fade-in 0.8s ease 0.9s forwards;
        }

        .hero-actions {
            display: flex;
            gap: 1rem;
            flex-wrap: wrap;
            justify-content: center;
            position: relative;
            z-index: 2;
            opacity: 0;
            animation: hero-fade-in 0.8s ease 1.1s forwards;
        }

        /* ── Buttons (override Bootstrap .btn) ── */
        .btn {
            display: inline-flex;
            align-items: center;
            gap: 0.5rem;
            padding: 0.9rem 2.2rem;
            border-radius: 10px;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.95rem;
            font-weight: 600;
            text-decoration: none;
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
            cursor: pointer;
            border: none;
            line-height: 1.5;
        }

        .btn-primary {
            background: linear-gradient(135deg, var(--crimson), var(--crimson-light)) !important;
            color: white !important;
            box-shadow: 0 4px 24px var(--crimson-glow);
            border: none !important;
        }

        .btn-primary:hover {
            transform: translateY(-3px);
            box-shadow: 0 8px 40px rgba(220, 20, 60, 0.55);
            color: white !important;
        }

        .btn-secondary {
            background: var(--surface) !important;
            color: var(--star-white) !important;
            border: 1px solid var(--border) !important;
        }

        .btn-secondary:hover {
            border-color: var(--border-hover) !important;
            background: var(--surface-hover) !important;
            transform: translateY(-3px);
            color: var(--star-white) !important;
        }

        .scroll-indicator {
            position: absolute;
            bottom: 2.5rem;
            left: 50%;
            transform: translateX(-50%);
            z-index: 2;
            animation: float 2.5s ease-in-out infinite;
            opacity: 0;
            animation: float 2.5s ease-in-out infinite, hero-fade-in 0.8s ease 1.8s forwards;
        }

        .scroll-indicator .mouse {
            width: 26px;
            height: 40px;
            border: 2px solid var(--crimson);
            border-radius: 13px;
            position: relative;
            opacity: 0.5;
        }

        .scroll-indicator .mouse::after {
            content: '';
            position: absolute;
            top: 8px;
            left: 50%;
            transform: translateX(-50%);
            width: 3px;
            height: 8px;
            background: var(--crimson);
            border-radius: 2px;
            animation: scroll-dot 1.8s ease-in-out infinite;
        }

        @keyframes scroll-dot {
            0%, 100% { opacity: 1; transform: translateX(-50%) translateY(0); }
            50% { opacity: 0.3; transform: translateX(-50%) translateY(10px); }
        }

        @keyframes float {
            0%, 100% { transform: translateX(-50%) translateY(0); }
            50% { transform: translateX(-50%) translateY(10px); }
        }

        /* ── Sections ── */
        .section-sep {
            width: 60px;
            height: 2px;
            background: linear-gradient(90deg, transparent, var(--crimson), transparent);
            margin: 0 auto 6rem;
            opacity: 0.5;
        }

        section {
            padding: 6rem 0;
            position: relative;
        }

        .section-header {
            text-align: center;
            margin-bottom: 4rem;
        }

        .section-label {
            display: inline-block;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.78rem;
            color: var(--crimson-light);
            text-transform: uppercase;
            letter-spacing: 0.18em;
            margin-bottom: 0.75rem;
            padding: 0.35rem 1.1rem;
            background: var(--crimson-dim);
            border: 1px solid rgba(220, 20, 60, 0.15);
            border-radius: 6px;
        }

        h2 {
            font-family: 'Inter', sans-serif;
            font-size: clamp(1.8rem, 4vw, 3rem);
            font-weight: 700;
            color: var(--star-white);
            margin-bottom: 1rem;
            letter-spacing: -0.03em;
        }

        .section-desc {
            color: var(--text-muted);
            max-width: 560px;
            margin: 0 auto;
            font-size: 1.05rem;
            line-height: 1.7;
        }

        /* ── Features grid (Bootstrap row + col) ── */
        .features-grid {
            --bs-gutter-x: 1.25rem;
            --bs-gutter-y: 1.25rem;
        }

        .feature-card {
            background: var(--surface);
            padding: 2rem;
            border-radius: 14px;
            border: 1px solid var(--border);
            transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1);
            position: relative;
            overflow: hidden;
            height: 100%;
        }

        .feature-card::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
            height: 2px;
            background: linear-gradient(90deg, transparent, var(--crimson), transparent);
            opacity: 0;
            transition: opacity 0.35s;
        }

        .feature-card::after {
            content: '';
            position: absolute;
            inset: 0;
            background: radial-gradient(ellipse at top left, rgba(220, 20, 60, 0.04), transparent 60%);
            opacity: 0;
            transition: opacity 0.35s;
        }

        .feature-card:hover {
            border-color: var(--border-hover);
            transform: translateY(-5px);
            background: var(--surface-hover);
            box-shadow: 0 12px 40px rgba(0, 0, 0, 0.3);
        }

        .feature-card:hover::before { opacity: 1; }
        .feature-card:hover::after { opacity: 1; }

        .feature-icon {
            font-size: 2.2rem;
            margin-bottom: 1.25rem;
            display: block;
            position: relative;
            z-index: 1;
        }

        .feature-card h3 {
            color: var(--star-white);
            margin-bottom: 0.6rem;
            font-size: 1.15rem;
            font-weight: 600;
            position: relative;
            z-index: 1;
        }

        .feature-card p {
            color: var(--text-muted);
            font-size: 0.95rem;
            line-height: 1.7;
            position: relative;
            z-index: 1;
        }

        /* ── Terminal mockup ── */
        .terminal-mockup {
            background: var(--space-dark);
            border-radius: 14px;
            border: 1px solid rgba(220, 20, 60, 0.2);
            overflow: hidden;
            max-width: 820px;
            margin: 0 auto;
            box-shadow: 0 24px 80px rgba(0, 0, 0, 0.5), 0 0 60px rgba(220, 20, 60, 0.06);
            position: relative;
        }

        .terminal-mockup::before {
            content: '';
            position: absolute;
            inset: -1px;
            border-radius: 15px;
            padding: 1px;
            background: linear-gradient(135deg, rgba(220, 20, 60, 0.2), transparent 50%, rgba(220, 20, 60, 0.1));
            -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0);
            -webkit-mask-composite: xor;
            mask-composite: exclude;
            pointer-events: none;
        }

        .terminal-header {
            background: rgba(220, 20, 60, 0.04);
            padding: 0.9rem 1.25rem;
            display: flex;
            align-items: center;
            gap: 0.5rem;
            border-bottom: 1px solid rgba(220, 20, 60, 0.1);
        }

        .terminal-dot {
            width: 12px;
            height: 12px;
            border-radius: 50%;
            transition: opacity 0.2s;
        }

        .terminal-mockup:hover .terminal-dot { opacity: 0.7; }

        .dot-red { background: #ff5f56; }
        .dot-yellow { background: #ffbd2e; }
        .dot-green { background: #27c93f; }

        .terminal-title {
            margin-left: 0.75rem;
            color: var(--text-dim);
            font-size: 0.8rem;
            font-family: 'JetBrains Mono', monospace;
        }

        .terminal-body {
            padding: 1.5rem;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.85rem;
            color: var(--terminal-green);
            line-height: 1.7;
        }

        .terminal-line { margin: 0.3rem 0; }

        .prompt { color: var(--crimson); margin-right: 0.5rem; }
        .command { color: var(--star-white); }
        .output { color: var(--text-muted); margin-left: 1.5rem; }
        .output-box { color: var(--crimson); margin-left: 1.5rem; opacity: 0.6; }
        .output-brand { color: var(--crimson-light); margin-left: 1.5rem; }
        .output-dim { color: var(--text-dim); margin-left: 1.5rem; }

        .cursor {
            display: inline-block;
            width: 9px;
            height: 18px;
            background: var(--terminal-green);
            animation: blink 1s step-end infinite;
            vertical-align: middle;
            margin-left: 2px;
        }

        @keyframes blink {
            0%, 50% { opacity: 1; }
            51%, 100% { opacity: 0; }
        }

        /* ── Stats row (Bootstrap row + col) ── */
        .stats-row {
            margin-top: 4rem;
            --bs-gutter-x: 1.5rem;
            --bs-gutter-y: 1.5rem;
        }

        .stat-item {
            text-align: center;
            padding: 2.2rem 1rem;
            background: var(--surface);
            border-radius: 14px;
            border: 1px solid var(--border);
            transition: all 0.3s ease;
        }

        .stat-item:hover {
            border-color: var(--border-hover);
            transform: translateY(-3px);
        }

        .stat-number {
            font-family: 'JetBrains Mono', monospace;
            font-size: clamp(2.2rem, 4vw, 3.2rem);
            font-weight: 700;
            background: linear-gradient(135deg, #ff6b81, var(--crimson-light), var(--crimson));
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-clip: text;
        }

        .stat-label {
            color: var(--text-muted);
            font-size: 0.85rem;
            text-transform: uppercase;
            letter-spacing: 0.1em;
            margin-top: 0.5rem;
        }

        /* ── Modes grid (Bootstrap row + col) ── */
        .modes-grid {
            margin-top: 3rem;
            --bs-gutter-x: 1.5rem;
            --bs-gutter-y: 1.5rem;
        }

        .mode-card {
            background: var(--surface);
            border-radius: 14px;
            border: 1px solid var(--border);
            padding: 2.5rem 2rem;
            text-align: center;
            transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1);
            position: relative;
            overflow: hidden;
            height: 100%;
        }

        .mode-card:hover {
            transform: translateY(-5px);
            border-color: var(--border-hover);
            box-shadow: 0 12px 40px rgba(0, 0, 0, 0.3);
        }

        .mode-card.featured {
            border-color: var(--crimson);
            background: linear-gradient(180deg, rgba(220, 20, 60, 0.08) 0%, var(--surface) 100%);
        }

        .mode-card.featured::before {
            content: '';
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
            height: 3px;
            background: linear-gradient(90deg, var(--crimson), var(--crimson-light), var(--crimson));
        }

        .mode-name {
            font-family: 'JetBrains Mono', monospace;
            font-size: 1.3rem;
            font-weight: 700;
            color: var(--crimson-light);
            margin-bottom: 0.5rem;
        }

        .mode-tagline {
            color: var(--star-white);
            font-size: 0.95rem;
            font-weight: 600;
            margin-bottom: 1rem;
        }

        .mode-desc {
            color: var(--text-muted);
            font-size: 0.9rem;
            line-height: 1.7;
        }

        .mode-icon {
            font-size: 2.5rem;
            margin-bottom: 1rem;
        }

        .mode-features {
            list-style: none;
            margin-top: 1.25rem;
            text-align: left;
            padding-left: 0;
        }

        .mode-features li {
            color: var(--text-muted);
            font-size: 0.88rem;
            padding: 0.35rem 0;
            display: flex;
            align-items: center;
            gap: 0.6rem;
        }

        .mode-features li::before {
            content: '\2713';
            color: var(--crimson-light);
            font-weight: 700;
            font-size: 0.85rem;
        }

        /* ── Agent table ── */
        .agent-table {
            width: 100%;
            max-width: 900px;
            margin: 3rem auto 0;
            border-collapse: separate;
            border-spacing: 0;
            border-radius: 14px;
            overflow: hidden;
            border: 1px solid var(--border);
        }

        .agent-table th {
            background: rgba(220, 20, 60, 0.08);
            color: var(--crimson-light);
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.82rem;
            font-weight: 600;
            text-transform: uppercase;
            letter-spacing: 0.1em;
            padding: 1rem 1.5rem;
            text-align: left;
            border-bottom: 1px solid var(--border);
        }

        .agent-table td {
            padding: 1rem 1.5rem;
            font-size: 0.92rem;
            color: var(--text-muted);
            border-bottom: 1px solid rgba(220, 20, 60, 0.06);
            background: var(--surface);
            transition: background 0.2s;
        }

        .agent-table tr:hover td {
            background: var(--surface-hover);
        }

        .agent-table tr:last-child td {
            border-bottom: none;
        }

        .agent-type-badge {
            font-family: 'JetBrains Mono', monospace;
            font-weight: 700;
            font-size: 0.88rem;
            padding: 0.25rem 0.75rem;
            border-radius: 6px;
            display: inline-block;
        }

        .badge-general { color: var(--crimson-light); background: rgba(220, 20, 60, 0.12); }
        .badge-explore { color: #00ff88; background: rgba(0, 255, 136, 0.1); }
        .badge-plan { color: #5b9aff; background: rgba(91, 154, 255, 0.1); }

        /* ── Quickstart code ── */
        .quickstart-code {
            background: var(--space-dark);
            border-radius: 14px;
            border: 1px solid var(--border);
            overflow: hidden;
            position: relative;
        }

        .quickstart-code::before {
            content: '';
            position: absolute;
            inset: -1px;
            border-radius: 15px;
            padding: 1px;
            background: linear-gradient(135deg, rgba(220, 20, 60, 0.15), transparent 50%);
            -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0);
            -webkit-mask-composite: xor;
            mask-composite: exclude;
            pointer-events: none;
        }

        .quickstart-code .terminal-body { padding: 2rem; }

        .code-comment { color: var(--text-dim); }
        .code-highlight { color: var(--crimson-light); }
        .code-string { color: var(--terminal-green); }

        /* ── Architecture flow ── */
        .arch-flow {
            display: flex;
            align-items: center;
            justify-content: center;
            gap: 0.5rem;
            flex-wrap: wrap;
            margin: 2rem auto;
            max-width: 900px;
        }

        .arch-node {
            background: var(--surface);
            border: 1px solid var(--border);
            padding: 0.7rem 1.2rem;
            border-radius: 10px;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.82rem;
            color: var(--star-white);
            transition: all 0.25s ease;
        }

        .arch-node:hover {
            border-color: var(--crimson);
            background: var(--surface-hover);
            transform: translateY(-2px);
        }

        .arch-node.core {
            border-color: var(--crimson);
            background: var(--crimson-dim);
            color: var(--crimson-light);
            box-shadow: 0 0 20px rgba(220, 20, 60, 0.15);
        }

        .arch-arrow { color: var(--text-dim); font-size: 1.2rem; }

        /* ── Back to top ── */
        .back-to-top {
            position: fixed;
            bottom: 2rem;
            right: 2rem;
            width: 44px;
            height: 44px;
            background: var(--surface);
            border: 1px solid var(--border);
            border-radius: 12px;
            color: var(--crimson);
            font-size: 1.2rem;
            cursor: pointer;
            z-index: 99;
            display: flex;
            align-items: center;
            justify-content: center;
            opacity: 0;
            transform: translateY(20px);
            transition: all 0.3s ease;
            pointer-events: none;
            backdrop-filter: blur(12px);
        }

        .back-to-top.visible {
            opacity: 1;
            transform: translateY(0);
            pointer-events: all;
        }

        .back-to-top:hover {
            border-color: var(--crimson);
            background: var(--surface-hover);
            transform: translateY(-3px);
        }

        /* ── Footer ── */
        footer {
            border-top: 1px solid var(--border);
            padding: 4rem 0;
            position: relative;
        }

        footer::before {
            content: '';
            position: absolute;
            top: -1px;
            left: 50%;
            transform: translateX(-50%);
            width: 120px;
            height: 2px;
            background: linear-gradient(90deg, transparent, var(--crimson), transparent);
        }

        .footer-inner {
            display: flex;
            align-items: center;
            justify-content: space-between;
            flex-wrap: wrap;
            gap: 1rem;
        }

        .footer-brand {
            font-family: 'JetBrains Mono', monospace;
            font-weight: 600;
            color: var(--text-muted);
            display: flex;
            align-items: center;
            gap: 0.5rem;
        }

        .footer-brand span { color: var(--crimson); }

        .footer-links {
            display: flex;
            gap: 1.5rem;
            list-style: none;
            padding-left: 0;
            margin-bottom: 0;
        }

        .footer-links a {
            color: var(--text-muted);
            text-decoration: none;
            font-size: 0.9rem;
            transition: color 0.2s;
        }

        .footer-links a:hover { color: var(--crimson-light); }

        .footer-copy {
            width: 100%;
            color: var(--text-dim);
            font-size: 0.8rem;
            margin-top: 1rem;
        }

        /* ── Scroll reveal ── */
        .reveal {
            opacity: 0;
            transform: translateY(30px);
            transition: opacity 0.7s cubic-bezier(0.4, 0, 0.2, 1), transform 0.7s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .reveal.visible {
            opacity: 1;
            transform: translateY(0);
        }

        .reveal-stagger .reveal-child {
            opacity: 0;
            transform: translateY(20px);
            transition: opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1), transform 0.5s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .reveal-stagger.visible .reveal-child {
            opacity: 1;
            transform: translateY(0);
        }

        .reveal-stagger.visible .reveal-child:nth-child(1) { transition-delay: 0s; }
        .reveal-stagger.visible .reveal-child:nth-child(2) { transition-delay: 0.1s; }
        .reveal-stagger.visible .reveal-child:nth-child(3) { transition-delay: 0.15s; }
        .reveal-stagger.visible .reveal-child:nth-child(4) { transition-delay: 0.2s; }
        .reveal-stagger.visible .reveal-child:nth-child(5) { transition-delay: 0.25s; }
        .reveal-stagger.visible .reveal-child:nth-child(6) { transition-delay: 0.3s; }

        /* ── Docs-specific styles ── */
        .docs-page .container { max-width: 1280px; }

        .docs-layout {
            padding-top: 7rem;
            padding-bottom: 4rem;
            min-height: 100vh;
            --bs-gutter-x: 3rem;
            align-items: start;
        }

        .docs-sidebar {
            position: sticky;
            top: 5rem;
        }

        .docs-nav {
            display: flex;
            flex-direction: column;
            gap: 0.15rem;
        }

        .docs-nav-heading {
            font-family: 'JetBrains Mono', monospace;
            font-weight: 700;
            font-size: 0.95rem;
            color: var(--star-white);
            text-decoration: none;
            padding: 0.5rem 1rem;
            margin-bottom: 0.75rem;
            transition: color 0.2s;
        }

        .docs-nav-heading:hover { color: var(--crimson-light); }

        .docs-nav-item {
            color: var(--text-muted);
            text-decoration: none;
            font-size: 0.88rem;
            padding: 0.45rem 1rem;
            border-left: 2px solid transparent;
            border-radius: 0 6px 6px 0;
            transition: all 0.2s;
        }

        .docs-nav-item:hover {
            color: var(--star-white);
            background: var(--crimson-subtle);
            border-left-color: rgba(220, 20, 60, 0.3);
        }

        .docs-nav-item.active {
            color: var(--crimson-light);
            background: var(--crimson-dim);
            border-left-color: var(--crimson);
            font-weight: 600;
        }

        .docs-content {
            max-width: 840px;
            line-height: 1.8;
        }

        .docs-content h1 {
            font-size: clamp(2rem, 4vw, 2.8rem);
            animation: none;
            opacity: 1;
            -webkit-text-fill-color: var(--star-white);
            background: none;
            margin-bottom: 0.5rem;
        }

        .docs-breadcrumb {
            font-size: 0.85rem;
            color: var(--text-dim);
            margin-bottom: 2rem;
        }

        .docs-breadcrumb a {
            color: var(--text-muted);
            text-decoration: none;
            transition: color 0.2s;
        }

        .docs-breadcrumb a:hover { color: var(--crimson-light); }

        .docs-content .lead {
            font-size: 1.1rem;
            color: var(--text-muted);
            margin-bottom: 2.5rem;
            line-height: 1.8;
        }

        .docs-content h2 {
            font-size: 1.6rem;
            margin-top: 3rem;
            margin-bottom: 1rem;
            padding-top: 1rem;
            border-top: 1px solid var(--border);
        }

        .docs-content h2:first-of-type {
            border-top: none;
            margin-top: 0;
            padding-top: 0;
        }

        .docs-content h3 {
            font-size: 1.2rem;
            font-weight: 600;
            color: var(--star-white);
            margin-top: 2rem;
            margin-bottom: 0.75rem;
        }

        .docs-content p {
            color: var(--text-muted);
            margin-bottom: 1.25rem;
        }

        .docs-content a {
            color: var(--crimson-light);
            text-decoration: none;
            border-bottom: 1px solid rgba(220, 20, 60, 0.3);
            transition: border-color 0.2s;
        }

        .docs-content a:hover {
            border-bottom-color: var(--crimson-light);
        }

        .docs-content code {
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.85em;
            color: var(--crimson-light);
            background: var(--crimson-dim);
            padding: 0.15rem 0.45rem;
            border-radius: 4px;
        }

        .docs-content pre {
            background: var(--space-dark);
            border: 1px solid var(--border);
            border-radius: 10px;
            padding: 1.25rem 1.5rem;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.85rem;
            line-height: 1.7;
            overflow-x: auto;
            margin-bottom: 1.5rem;
            color: var(--star-white);
        }

        .docs-content pre code {
            background: none;
            padding: 0;
            color: inherit;
            font-size: inherit;
        }

        .docs-content ul, .docs-content ol {
            color: var(--text-muted);
            padding-left: 1.5rem;
            margin-bottom: 1.25rem;
        }

        .docs-content li {
            margin-bottom: 0.4rem;
        }

        /* Docs tables: Bootstrap table-responsive wrapper + custom theme */
        .docs-content .table-responsive {
            -webkit-overflow-scrolling: touch;
            margin-bottom: 1.5rem;
        }

        .docs-content .table-wrap {
            overflow-x: auto;
            -webkit-overflow-scrolling: touch;
            margin-bottom: 1.5rem;
        }

        .docs-content table {
            width: 100%;
            min-width: 500px;
            border-collapse: separate;
            border-spacing: 0;
            border-radius: 10px;
            overflow: hidden;
            border: 1px solid var(--border);
            margin-bottom: 0;
        }

        .docs-content th {
            background: rgba(220, 20, 60, 0.08);
            color: var(--crimson-light);
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.8rem;
            font-weight: 600;
            text-transform: uppercase;
            letter-spacing: 0.08em;
            padding: 0.75rem 1rem;
            text-align: left;
            border-bottom: 1px solid var(--border);
        }

        .docs-content td {
            padding: 0.7rem 1rem;
            font-size: 0.9rem;
            color: var(--text-muted);
            border-bottom: 1px solid rgba(220, 20, 60, 0.06);
            background: var(--surface);
        }

        .docs-content tr:last-child td { border-bottom: none; }

        .docs-content .tip {
            background: rgba(220, 20, 60, 0.06);
            border-left: 3px solid var(--crimson);
            padding: 1rem 1.25rem;
            border-radius: 0 8px 8px 0;
            margin-bottom: 1.5rem;
        }

        .docs-content .tip p {
            margin-bottom: 0;
            font-size: 0.92rem;
        }

        .docs-content .tip strong {
            color: var(--crimson-light);
        }

        /* ── Pipeline visualization ── */
        .pipeline-flow {
            display: flex;
            gap: 0;
            align-items: stretch;
            overflow-x: auto;
            -webkit-overflow-scrolling: touch;
            padding: 1rem 0;
            margin-bottom: 1.5rem;
        }

        .pipeline-stage {
            flex: 1;
            min-width: 140px;
            background: var(--surface);
            border: 1px solid var(--border);
            border-radius: 10px;
            padding: 1.25rem 1rem;
            text-align: center;
        }

        .pipeline-stage h4 {
            color: var(--crimson-light);
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.8rem;
            font-weight: 700;
            margin-bottom: 0.5rem;
        }

        .pipeline-stage p {
            color: var(--text-muted);
            font-size: 0.78rem;
            line-height: 1.4;
            margin: 0;
        }

        .pipeline-stage .stage-tag {
            display: inline-block;
            margin-top: 0.5rem;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.68rem;
            color: var(--text-dim);
            background: var(--crimson-subtle);
            padding: 0.15rem 0.5rem;
            border-radius: 4px;
        }

        .pipeline-arrow {
            display: flex;
            align-items: center;
            color: var(--crimson);
            font-size: 1.2rem;
            padding: 0 0.35rem;
            flex-shrink: 0;
        }

        /* ── Budget bar ── */
        .budget-bar {
            background: var(--surface);
            border: 1px solid var(--border);
            border-radius: 10px;
            padding: 1.5rem;
            margin-bottom: 1.5rem;
        }

        .budget-bar-header {
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.85rem;
            color: var(--star-white);
            margin-bottom: 1rem;
        }

        .budget-bar-track {
            height: 32px;
            background: var(--space-dark);
            border-radius: 6px;
            overflow: hidden;
            display: flex;
            margin-bottom: 0.75rem;
        }

        .budget-segment {
            height: 100%;
            display: flex;
            align-items: center;
            justify-content: center;
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.65rem;
            font-weight: 600;
            color: white;
            white-space: nowrap;
            overflow: hidden;
        }

        .budget-legend {
            display: flex;
            flex-wrap: wrap;
            gap: 0.75rem 1.5rem;
        }

        .budget-legend-item {
            display: flex;
            align-items: center;
            gap: 0.4rem;
            font-size: 0.78rem;
            color: var(--text-muted);
        }

        .budget-legend-dot {
            width: 10px;
            height: 10px;
            border-radius: 3px;
            flex-shrink: 0;
        }

        @media (max-width: 767.98px) {
            .pipeline-flow { flex-direction: column; }
            .pipeline-arrow { transform: rotate(90deg); justify-content: center; padding: 0.3rem 0; }
        }

        /* ── Install tabs ── */
        .install-tabs {
            display: flex;
            gap: 0;
            margin-bottom: 0;
            border-bottom: 2px solid var(--border);
        }

        .install-tab {
            padding: 0.75rem 1.5rem;
            background: none;
            border: none;
            color: var(--text-muted);
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.88rem;
            font-weight: 600;
            cursor: pointer;
            border-bottom: 2px solid transparent;
            margin-bottom: -2px;
            transition: all 0.2s;
        }

        .install-tab:hover { color: var(--star-white); }

        .install-tab.active {
            color: var(--crimson-light);
            border-bottom-color: var(--crimson);
        }

        .install-panel {
            display: none;
        }

        .install-panel.active {
            display: block;
        }

        /* ── Docs index cards ── */
        .docs-index-grid {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
            gap: 1.25rem;
            margin-top: 2rem;
        }

        .docs-card {
            background: var(--surface);
            padding: 1.75rem;
            border-radius: 14px;
            border: 1px solid var(--border);
            text-decoration: none;
            transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1);
            display: block;
        }

        .docs-card:hover {
            border-color: var(--border-hover);
            transform: translateY(-4px);
            background: var(--surface-hover);
            box-shadow: 0 12px 40px rgba(0, 0, 0, 0.3);
        }

        .docs-card h3 {
            color: var(--star-white);
            font-size: 1.1rem;
            font-weight: 600;
            margin-bottom: 0.5rem;
        }

        .docs-card p {
            color: var(--text-muted);
            font-size: 0.9rem;
            line-height: 1.6;
            margin: 0;
        }

        .docs-card .card-icon {
            font-size: 1.8rem;
            margin-bottom: 0.75rem;
            display: block;
        }

        /* ── Mobile docs toggle ── */
        .docs-mobile-toggle {
            display: none;
            background: var(--surface);
            border: 1px solid var(--border);
            border-radius: 10px;
            padding: 0.75rem 1rem;
            color: var(--star-white);
            font-family: 'JetBrains Mono', monospace;
            font-size: 0.88rem;
            cursor: pointer;
            width: 100%;
            text-align: left;
            margin-bottom: 1rem;
        }

        /* ── Responsive ── */
        @media (max-width: 767.98px) {
            .hero { padding: 7rem 1.5rem 3rem; }

            .arch-flow { flex-direction: column; }
            .arch-arrow { transform: rotate(90deg); }
            .container { padding: 0 1.5rem; }
            .footer-inner { flex-direction: column; text-align: center; }

            .agent-table { font-size: 0.82rem; }
            .agent-table th, .agent-table td { padding: 0.75rem 1rem; }
        }

        @media (max-width: 991.98px) {
            /* Docs responsive: sidebar collapses below lg */
            .docs-layout {
                padding-top: 5.5rem;
            }

            .docs-sidebar {
                position: relative;
                top: auto;
            }

            .docs-sidebar .docs-nav {
                display: none;
                background: var(--surface);
                border: 1px solid var(--border);
                border-radius: 10px;
                padding: 0.75rem;
                margin-bottom: 1.5rem;
            }

            .docs-sidebar .docs-nav.open { display: flex; }
            .docs-mobile-toggle { display: block; }

            .docs-content pre {
                font-size: 0.78rem;
                padding: 1rem;
            }

            .docs-content th, .docs-content td {
                padding: 0.5rem 0.7rem;
                font-size: 0.82rem;
            }
        }

        @media print {
            #starfield, .navbar-kosmo, .scroll-indicator, .back-to-top { display: none; }
            body { background: white; color: black; }
            .terminal-mockup, .feature-card, .stat-item, .mode-card { border: 1px solid #ccc; background: white; }
        }
    </style>
</head>
<body class="<?= $pageClass ?>">
    <canvas id="starfield"></canvas>

    <nav class="navbar navbar-dark navbar-expand-lg fixed-top navbar-kosmo">
        <div class="container-xl" style="max-width: 1200px;">
            <a href="/" class="navbar-brand nav-brand">KosmoKrator</a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Menu">
                <span class="navbar-toggler-icon">
                    <span class="toggler-bar"></span>
                    <span class="toggler-bar"></span>
                    <span class="toggler-bar"></span>
                </span>
            </button>
            <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
                <ul class="navbar-nav">
                    <li class="nav-item"><a class="nav-link" href="<?= $isDocsPage ? '/' : '' ?>#features">Features</a></li>
                    <li class="nav-item"><a class="nav-link" href="<?= $isDocsPage ? '/' : '' ?>#quickstart">Install</a></li>
                    <li class="nav-item"><a class="nav-link<?= $isDocsPage ? ' active' : '' ?>" href="/docs">Docs</a></li>
                    <li class="nav-item"><a class="nav-link" href="https://github.com/OpenCompanyApp/kosmokrator" target="_blank" rel="noopener">GitHub</a></li>
                </ul>
            </div>
        </div>
    </nav>

    <button class="back-to-top" aria-label="Back to top">&#8593;</button>

    <div class="container">
        <?= $pageBody ?>

        <footer>
            <div class="footer-inner">
                <div class="footer-brand">KosmoKrator</div>
                <ul class="footer-links">
                    <li><a href="https://github.com/OpenCompanyApp/kosmokrator">GitHub</a></li>
                    <li><a href="/docs">Docs</a></li>
                    <li><a href="/docs/agents">Agents</a></li>
                    <li><a href="/docs/permissions">Permissions</a></li>
                </ul>
                <div class="footer-copy">&copy; OpenCompany &middot; MIT License</div>
            </div>
        </footer>
    </div>

    <!-- Bootstrap 5 JS Bundle -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>

    <script>
        // Starfield
        (function() {
            const canvas = document.getElementById('starfield');
            const ctx = canvas.getContext('2d');
            let stars = [];
            let shootingStars = [];
            let w, h;

            function resize() {
                w = canvas.width = window.innerWidth;
                h = canvas.height = window.innerHeight;
            }

            function initStars() {
                stars = [];
                const count = Math.floor((w * h) / 4000);
                for (let i = 0; i < count; i++) {
                    stars.push({
                        x: Math.random() * w,
                        y: Math.random() * h,
                        r: Math.random() * 1.5 + 0.3,
                        a: Math.random(),
                        da: (Math.random() - 0.5) * 0.008,
                        speed: Math.random() * 0.15 + 0.02
                    });
                }
            }

            function spawnShootingStar() {
                if (Math.random() > 0.003) return;
                shootingStars.push({
                    x: Math.random() * w * 0.8,
                    y: Math.random() * h * 0.4,
                    len: Math.random() * 80 + 40,
                    speed: Math.random() * 6 + 4,
                    angle: Math.PI / 4 + (Math.random() - 0.5) * 0.3,
                    life: 1
                });
            }

            let mouseX = 0, mouseY = 0;
            document.addEventListener('mousemove', (e) => {
                mouseX = (e.clientX / w - 0.5) * 10;
                mouseY = (e.clientY / h - 0.5) * 10;
            }, { passive: true });

            function draw() {
                ctx.clearRect(0, 0, w, h);
                for (const s of stars) {
                    s.a += s.da;
                    if (s.a > 1 || s.a < 0.1) s.da = -s.da;
                    s.a = Math.max(0.1, Math.min(1, s.a));
                    ctx.beginPath();
                    ctx.arc(s.x + mouseX * s.speed, s.y + mouseY * s.speed, s.r, 0, Math.PI * 2);
                    ctx.fillStyle = `rgba(255,255,255,${s.a * 0.6})`;
                    ctx.fill();
                }
                spawnShootingStar();
                for (let i = shootingStars.length - 1; i >= 0; i--) {
                    const ss = shootingStars[i];
                    ss.x += Math.cos(ss.angle) * ss.speed;
                    ss.y += Math.sin(ss.angle) * ss.speed;
                    ss.life -= 0.015;
                    if (ss.life <= 0) { shootingStars.splice(i, 1); continue; }
                    const tailX = ss.x - Math.cos(ss.angle) * ss.len;
                    const tailY = ss.y - Math.sin(ss.angle) * ss.len;
                    const grad = ctx.createLinearGradient(tailX, tailY, ss.x, ss.y);
                    grad.addColorStop(0, 'transparent');
                    grad.addColorStop(1, `rgba(220, 20, 60, ${ss.life * 0.7})`);
                    ctx.beginPath();
                    ctx.moveTo(tailX, tailY);
                    ctx.lineTo(ss.x, ss.y);
                    ctx.strokeStyle = grad;
                    ctx.lineWidth = 1.5;
                    ctx.stroke();
                }
                requestAnimationFrame(draw);
            }

            resize();
            initStars();
            draw();
            window.addEventListener('resize', () => { resize(); initStars(); }, { passive: true });
        })();

        // Nav scroll
        const navEl = document.querySelector('.navbar-kosmo');
        window.addEventListener('scroll', () => {
            navEl.classList.toggle('scrolled', window.scrollY > 50);
        }, { passive: true });

        // Close Bootstrap collapse on nav link click (mobile)
        const navbarCollapse = document.getElementById('navbarNav');
        if (navbarCollapse) {
            navbarCollapse.querySelectorAll('.nav-link').forEach(a => {
                a.addEventListener('click', () => {
                    const bsCollapse = bootstrap.Collapse.getInstance(navbarCollapse);
                    if (bsCollapse) bsCollapse.hide();
                });
            });
        }

        // Scroll reveal
        const reveals = document.querySelectorAll('.reveal, .reveal-stagger');
        if (reveals.length) {
            const observer = new IntersectionObserver((entries) => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        entry.target.classList.add('visible');
                        observer.unobserve(entry.target);
                    }
                });
            }, { threshold: 0.08, rootMargin: '0px 0px -30px 0px' });
            reveals.forEach(el => observer.observe(el));
        }

        // Back to top
        const backBtn = document.querySelector('.back-to-top');
        window.addEventListener('scroll', () => {
            backBtn.classList.toggle('visible', window.scrollY > 600);
        }, { passive: true });
        backBtn.addEventListener('click', () => {
            window.scrollTo({ top: 0, behavior: 'smooth' });
        });

        // Docs mobile sidebar toggle
        const docsToggle = document.querySelector('.docs-mobile-toggle');
        if (docsToggle) {
            docsToggle.addEventListener('click', () => {
                const nav = docsToggle.nextElementSibling;
                nav.classList.toggle('open');
            });
        }

        // Auto-wrap docs tables with Bootstrap table-responsive
        document.querySelectorAll('.docs-content table').forEach(table => {
            if (!table.parentElement.classList.contains('table-responsive') &&
                !table.parentElement.classList.contains('table-wrap')) {
                const wrapper = document.createElement('div');
                wrapper.className = 'table-responsive';
                table.parentNode.insertBefore(wrapper, table);
                wrapper.appendChild(table);
            }
        });
    </script>

    <?= $extraScript ?>
</body>
</html>
