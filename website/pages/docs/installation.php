...
<ul>
    ...
    <li>Extensions: <code>pcntl</code>, <code>posix</code>, <code>mbstring</code></li>
</ul>
...
<div class="tip">
    <p>
        ...
        installed, run <code>php -m | grep -E 'pcntl|posix|mbstring'</code>.
        All three should appear in the output. On most systems they are
...
<ol>
    <li>
        <strong>Provider selection</strong> &mdash; Choose from 40+ supported
    ...
    <li>
        ...
        chosen provider. Keys are stored locally and never transmitted anywhere
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                Run without interactive input. Designed for CI pipelines,
                automated scripts, and non-interactive environments.
                KosmoKrator reads the prompt from stdin or the
                <code>--prompt</code> flag and exits when the agent finishes.
            </td>
        </tr>
    </tbody>
</table>
...
<pre><code>php vendor/bin/box compile</code></pre>

<p>
    The compiled PHAR will be written to the project root. You can then copy
    it to a location on your <code>$PATH</code> and use it exactly like the
    release PHAR.
</p>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td><code>posix</code> extension</td>
            <td>Bundled</td>
            <td>Required</td>
            <td>Required</td>
        </tr>
...
<pre><code>sudo apt install php8.4-pcntl php8.4-posix php8.4-mbstring</code></pre>
