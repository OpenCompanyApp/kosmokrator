...
| `src/LLM/` | Native LLM stack: `AsyncLlmClient`, `RetryableLlmClient`, provider registry, Codex OAuth, provider catalog, model definitions, pricing service |
...
| --- | --- | --- | --- |
| 1 (lowest) | Bundled defaults | `config/kosmo.yaml` | Sane baseline defaults shipped with the application |
| 2 | User global | `~/.kosmo/config.yaml` | Personal overrides: API keys, preferred model, theme |
| 3 | Project-local | `.kosmo.yaml` or `.kosmo/config.yaml` | Per-project overrides: different model, plan mode default |
