...
This is the reference page for commands run from a shell. Interactive slash commands and power commands live in [Commands](/docs/commands); this page covers the Symfony Console command surface exposed by `kosmo` and `bin/kosmo`.
...
```bash
kosmo setup
kosmo setup --provider openai --model gpt-5.5 --api-key-env OPENAI_API_KEY --global --json
...
kosmo config set agent.default_model gpt-5.5 --provider openai --global --json
...
printf %s "$OPENAI_API_KEY" | \
  kosmo providers:configure openai --api-key-stdin --model gpt-5.5 --global --json
