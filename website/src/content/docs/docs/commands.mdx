...
Set a persistent goal for the current session. While an active goal exists in Edit mode, KosmoKrator continues working across agent turns until the goal is completed, paused, cleared, or reaches its configured token budget.

```bash
/goal finish the release prep and verify CI
/goal
/goal pause
/goal resume
/goal clear
...
kosmo config set agent.default_model glm-5.2 --provider z --global --json
...
jq -n '{settings:{"agent.default_provider":"openai","agent.default_model":"gpt-5.5"}}' | \
...
printf %s "$OPENAI_API_KEY" | \
  kosmo providers:configure openai --model gpt-5.5 --api-key-stdin --global --json
