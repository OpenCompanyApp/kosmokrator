...
1.  **Bundled defaults** — `config/kosmo.yaml` inside the application. Ships with sane defaults for every setting. You never need to edit this file; it exists as the baseline.
...
kosmo settings:set agent.default_model gpt-5.5 --provider openai --global --json
...
| `agent.reasoning_effort` | choice: `off`, `low`, `medium`, `high`, `max` | `max` | Controls extended thinking / chain-of-thought reasoning for models that support it. GLM supports `high` and `max`; `off` disables GLM thinking explicitly. | applies now |
...
| `tools.bash.background_wait_ms` | number | `250` | Initial output wait, in milliseconds, for background bash tool calls. | next session |
...
| `kosmo.mcp.servers.SERVER.permissions.read` | choice: `allow`, `ask`, `deny` | _inherits default_ | Read policy for one MCP server. Also applies to resources and prompts. | applies now |
| `kosmo.mcp.servers.SERVER.permissions.write` | choice: `allow`, `ask`, `deny` | _inherits default_ | Write policy for one MCP server. Tools without MCP `readOnlyHint` are treated as write. | applies now |
| `kosmo.mcp.trust.SERVER.fingerprint` | string | _unset_ | Fingerprint written by `mcp:trust` after reviewing a project MCP command. | applies now |
