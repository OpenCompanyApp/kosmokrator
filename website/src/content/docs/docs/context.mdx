...
The pipeline is designed so that most sessions never reach the later stages. Output truncation and deduplication handle the bulk of token reduction silently, keeping the conversation lean without any loss of important context. Use `/context` at any time to inspect the live token budget, cache stats, largest context items, and suggestions.
...
Each candidate tool result is scored by prune priority. Higher-priority results are pruned first. The score factors include:

-   **Tool type priority** — Tools with typically larger, less reusable output score higher. Shell output is pruned before referenced file reads when both are old.
-   **Reference by later reasoning** — If an assistant message after the tool result references the file name or quotes part of the result, the priority is reduced, making it more likely to be kept.
-   **Decision language** — If subsequent assistant messages contain phrases like "based on", "I'll use", or "the issue is" that suggest the tool result influenced a decision, the priority is reduced.
...
5.  The inserted summary is wrapped as historical reference only: the latest user message, current mode, current system prompt, and current tool results always override it.
6.  A bounded **Current Working State** block restores recent files, active tasks, and active background work without rehydrating large outputs.
7.  The summary is also stored as a **working memory** (with a 14-day expiration) so that the context persists even if the session ends.
8.  Durable memories are extracted from the same compaction response — facts about the codebase, user preferences, and technical decisions. These are saved permanently for cross-session recall.
...
The `/context` slash command shows live harness diagnostics:

-   Model, context window, effective window, and thresholds
-   Estimated tokens used and largest buckets
-   Largest individual messages and tool results
-   Last provider prompt-cache read/write tokens
-   File-read and web transient cache hits, misses, evictions, and entries
-   Suggestions for large shell output, memory bloat, high context pressure, or cache-read drops

Use `/context --full` for more cache detail or `/context --json` for machine-readable output.
