...
| MCP | `mcp.permissions_default`, `mcp.servers.SERVER.permissions.read`, `.write`, and `mcp.trust.SERVER.fingerprint` | `--force` bypasses MCP project trust and MCP read/write policy for the one headless call. |
